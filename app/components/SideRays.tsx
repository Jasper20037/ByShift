...
const SideRays = ({
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      while (containerRef.current.firstChild) containerRef.current.firstChild.remove();
  ...
  return (
    <div
      ref={containerRef}
      className={`w-full h-full overflow-hidden pointer-events-none ${className}`.trim()}
    />
  );
};
